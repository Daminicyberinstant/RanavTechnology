...
const Navbar = () => {
  ...
  return (
    <>
      {/* Overlay */}
      {isOpen && <div className="fixed inset-0 bg-black bg-opacity-50 z-[100] md:hidden" onClick={closeMobileMenu} />}

      {/* Header Bar */}
      <nav className="lg:w-[75vw] md:w-[90vw] w-[95vw] mx-auto mt-5 px-4 py-3 border border-gray-300 bg-white  rounded-xl flex items-center justify-between z-50 relative">
        {/* Logo */}
          ...
          {navigationLinks.map((link) => {
            ...
            return (
              <li key={link.href}>
                <Link
                  href={link.href}
                  className={`hover:text-[#016e7c] transition-colors duration-200 ${isActive ? 'text-[#016e7c]' : ''
                    }`}
                  style={{
      ...
      <div
        id="mobile-sidebar"
        className={`fixed top-0 left-0 h-screen w-[85%] max-w-sm bg-white z-[200] shadow-lg transition-transform duration-300 ease-in-out ${isOpen ? "translate-x-0" : "-translate-x-full"
          }`}
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <nav className="mt-6 px-6">
          ...
          <ul className="text-sm font-medium text-black uppercase">
            {/* Home Link */}
            <li>
              <Link
                href="/"
